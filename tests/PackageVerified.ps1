...
foreach($section in @('UI Micro Polish Hands-on Candidate','## 配置パネルの簡易設定・固定列・位置ショートカット','## 汎用配置のレイヤーをすばやく指定する','## 表情をまとめて：行クリックと即時反映','## Excelと未配置作業の保護','⚙ 簡易設定','マウスホイールで±1','数字キーを押すだけでレイヤー入力を開始','↕ グリップ','今回の変更を戻す','一覧を読み直す')) {
