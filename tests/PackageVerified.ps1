...
foreach ($stage in @('P1','P2','P3','P4','P5','P6','P7','P8','P9','W3','W4','W5','W6','W7','W8','W9','W10','W11','W12_UI','W12','V04')) {
...
$expected=@('Ymm4TemplatePlacer.dll','Ymm4TemplatePlacer.deps.json','DocumentFormat.OpenXml.dll','DocumentFormat.OpenXml.Framework.dll','README.md','THIRD_PARTY_NOTICES.md','provenance.json','v04-acceptance.json')
...
try {
 foreach ($name in @('AGENTS.md','docs/DESIGN.md','docs/USAGE.md','src/Ymm4TemplatePlacer/Ymm4TemplatePlacer.csproj','tests/NativeV04Proof.cs','tests/NativeAcceptanceProof.cs','tests/PackageVerified.ps1')) {
  if ($null -eq $sourceZip.GetEntry($name)) { throw "Missing source archive entry: $name" }
 }
} finally { $sourceZip.Dispose() }
[ordered]@{ result='PASS'; version=$version; native_stages='P1-P9,W3-W12,V04'; acceptance_requirements=18; payload_files=$expected; archived_dll_sha256=$archivedHash; source_archive='Ymm4TemplatePlacer-source.zip' } |
