...
$round3 = & "$PSScriptRoot/ValidateRound3Evidence.ps1" -OutputDir $OutputDir -SelfTest
$round3Guard=Get-Content -Raw (Join-Path $OutputDir 'round3-evidence-guard-tests.json') | ConvertFrom-Json
if ($round3Guard.schema -cne 'YMM4-Template-Placer-Round3-Evidence-Guard/1' -or $round3Guard.result -cne 'PASS' -or $round3Guard.positive_checks -ne 1 -or $round3Guard.negative_checks -ne 20 -or @($round3Guard.checks).Count -ne 20 -or @($round3Guard.checks | Where-Object {$_.result -cne 'PASS_REJECTED'}).Count) {throw 'Round3 evidence guard self-test is incomplete'}
...
$stages=@('PORTABLE_SETTINGS','PLACEMENT_SOURCE_P0','PLACEMENT_SOURCE_P1','P1','P2','P3','P4','P5','P6','P7','P8','P9','W3','W4','W5','W6','W7','W8','W9','W10','W11','W12_UI','W12_SELECTORS','W12','V04') + (1..13 | ForEach-Object { "WUX$_" }) + @('UX_ACCEPTANCE','UX_WORKFLOW_ACCEPTANCE','TEMPLATE_FIDELITY','HANDS_ON_H1_H2','HANDS_ON_H3_H4_H5','HANDS_ON_UX_POLISH','HANDS_ON_ROUND2_A','HANDS_ON_ROUND2_B','HANDS_ON_ROUND2_C','HANDS_ON_ROUND2_D','HANDS_ON_ROUND2_E','HANDS_ON_ROUND2')
...
$sourceTree=git rev-parse "${sourceHead}^{tree}"
if($LASTEXITCODE -ne 0 -or $sourceTree -cne (git rev-parse 'HEAD^{tree}')) {throw 'Source HEAD and CI checkout trees differ'}
[ordered]@{
 ...
 hands_on_round3_version=$round3.version; hands_on_round3_result=$round3.result; hands_on_round3_native_requirements=@($round3.checks).Count
 round3_evidence_guard_negative_checks=$round3Guard.negative_checks; total_evidence_rejection_checks=($guard.negative_checks+$round3Guard.negative_checks)
