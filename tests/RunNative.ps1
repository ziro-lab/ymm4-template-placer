...
if (-not (Select-String -Path $log -Pattern '^PLACEMENT_SOURCE_P7=PASS
...
)) { throw 'Placement Source P7 proof is incomplete' }
if (-not (Select-String -Path $log -Pattern '^PLACEMENT_RULE_P1=PASS
...
if ($workflow.version -ne '0.5.0' -or $workflow.result -ne 'PASS' -or @($workflow.checks).Count -ne 10 -or @($workflow.checks | Where-Object { $_.result -ne 'PASS' }).Count) { throw 'Incomplete v0.5.0 UX workflow acceptance evidence' }
Write-Host 'Checkpoint native validation: full semantic regression and evidence guards PASS'
)) { throw 'Placement Rule P1 proof is incomplete' }
if (-not (Select-String -Path $log -Pattern '^PLACEMENT_RULE_P2=PASS
...
if ($workflow.version -ne '0.5.0' -or $workflow.result -ne 'PASS' -or @($workflow.checks).Count -ne 10 -or @($workflow.checks | Where-Object { $_.result -ne 'PASS' }).Count) { throw 'Incomplete v0.5.0 UX workflow acceptance evidence' }
Write-Host 'Checkpoint native validation: full semantic regression and evidence guards PASS'
)) { throw 'Placement Rule P2 proof is incomplete' }
