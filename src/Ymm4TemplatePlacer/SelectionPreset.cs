...
public enum SelectionProfile { TargetCompanion, PointEmphasis, SelectionRange }
    ...
    int StartOffset, int EndOffset, int AnchorPercent, int Duration, LayerPolicy Layer)
{
    ...
    public void Validate()
    {
        if (Id == Guid.Empty || string.IsNullOrWhiteSpace(Name) || Name.Length > 128 || !Enum.IsDefined(Profile) ||
            AnchorPercent is not (0 or 25 or 50 or 75 or 100) || Duration < 1 || HeadPadding < 0 || TailPadding < 0 || Layer == null)
            throw new InvalidOperationException("選択配置Presetが不正です。長さは1frame以上、前後の余白は0以上にしてください。");
        Layer.Validate();
    }
}
...
public sealed partial class PlacerSettings
{
    ...
    public List<SelectionPreset> SelectionPresets { get; set; } = [SelectionPreset.Companion, SelectionPreset.Emphasis, SelectionPreset.Range];
...
public static class SelectionPresetSettings
{
    ...
    {
        if (settings.SelectionPresetRevision != 0 || settings.SelectionPresets == null || settings.SelectionPresets.Any(x => x == null)) return;
    ...
    {
        if (settings.SelectionPresetRevision is < 0 or > 1 || settings.SelectionPresets == null || settings.SelectionPresets.Count is < 3 or > 128 ||
