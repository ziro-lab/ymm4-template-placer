...
public static class SelectionRangeProfile
{
    ...
    {
        if (targets.Count < 2) throw new InvalidOperationException("範囲配置には2つ以上のアイテムを選択してください。");
...
public sealed record SelectionPlacement(IItem Item, PlacementPlan Plan)
{
    public static (int Frame, int Length) CheckedSpan(long start, long end)
    {
        if (start < 0 || end <= start || end > int.MaxValue)
            throw new InvalidOperationException("配置範囲が不正です。開始は0以上、長さは1フレーム以上、終了は整数範囲内にしてください。タイムラインは変更していません。");
    ...
    public static IReadOnlyList<SelectionProfileChoice> Profiles(int count) => count switch
    {
        ...
        2 => [new(SelectionProfile.SelectionRange, "選択範囲を覆う"), new(SelectionProfile.Boundary, "2アイテムの境界（許容差を確認）")],
    ...
    public static SelectionPlacement Create(Timeline timeline, LibraryEntry entry, SelectionPreset preset)
    {
        ...
        if (!Supports(preset.Profile, targets.Length) || targets.Distinct().Count() != targets.Length)
            throw new InvalidOperationException("この配置方法に必要な数のアイテムをタイムラインで選択してください。対象・基準点は1つ、範囲は2つ以上、境界は2つだけです。");
        foreach (var target in targets)
        {
            if (!timeline.Items.Contains(target)) throw new InvalidOperationException("選択したアイテムが現在のタイムラインにありません。選び直してください。");
        ...
        var span = preset.Profile switch
        {
            ...
            _ => throw new InvalidOperationException("未対応の配置方法です。")
        };
        ...
        foreach (var target in targets)
        {
            ...
            if (character != null && ((sourceCharacter != null && !Equals(sourceCharacter, character)) ||
                ...
                throw new InvalidOperationException("テンプレートと対象のキャラクターが違います。キャラクターを自動で書き換えず停止しました。");
        }
