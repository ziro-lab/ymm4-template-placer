...
public sealed record LayerPolicy
{
    ...
    {
        if (!Enum.IsDefined(SearchMode) || (SearchMode != LayerSearchMode.Legacy && UseTemplateLayer))
            throw new InvalidOperationException("数値レイヤー指定の探索設定が不正です。");
        ...
        Character? character, IEnumerable<IItem> occupancy, IItem? ignore = null)
    {
        ...
        if (policy.SearchMode != LayerSearchMode.Legacy)
        {
            ...
            if (policy.SearchMode == LayerSearchMode.SearchUp)
                for (var layer = policy.Preferred - 1; layer >= policy.Minimum; layer--) if (Free(layer)) return layer;
            ...
            throw new InvalidOperationException(policy.SearchMode == LayerSearchMode.DoNotPlace
                ? $"指定レイヤー {policy.Preferred} は予定の長さの途中を含めて使用中です。配置していません。"
