...
{
    public static ResyncPlan Create(Timeline timeline, ExpressionPreset preset, IReadOnlyList<IItem>? selectedItems = null)
    {
        preset.Validate();
        var candidates = new HashSet<TachieFaceItem>(); var skipped = new List<string>(); var ignored = 0;
        foreach (var selected in (selectedItems ?? timeline.SelectedItems.ToArray()).Distinct())
        {
            ...
            if (selected is VoiceItem voice)
            {
                ...
                foreach (var face in timeline.Items.OfType<TachieFaceItem>().Where(x =>
                    (Equals(x.Character, voice.Character) || IntentAssociationTag.Read(x.Remark, out _) != AssociationTagState.None && x.Character?.Name == voice.Character?.Name) &&
                    AssociationTag.Source(x.Remark, out var source) == AssociationTagState.Valid && source!.Serial == id)) candidates.Add(face);
            }
            else if (selected is TachieFaceItem face && AssociationTag.Source(face.Remark, out _) != AssociationTagState.None) candidates.Add(face);
            else ignored++;
        }
        var snapshots = VoiceSnapshot.Capture(timeline); var occupancy = timeline.Items.ToList();
        var updates = new List<PlannedItemUpdate>(); var unchanged = 0;
        foreach (var face in timeline.Items.OfType<TachieFaceItem>().Where(candidates.Contains).OrderBy(x => x.Frame).ThenBy(x => x.Layer))
        {
            try
            {
                if (IntentAssociationTag.Read(face.Remark, out _) != AssociationTagState.None)
                    throw new InvalidOperationException("相対パレットの関連表情です。相対パレット側の再同期を使ってください");
                ...
                updates.Add(new(face, span.Frame, span.Length, layer, face.Remark ?? "")); occupancy.Remove(face); occupancy.Add(reservation);
            }
